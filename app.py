...
app.config['MYSQL_PASSWORD'] = '12212'
app.config['MYSQL_DB'] = 'flask01'
...
def process_lis_dept_file(file2_path):
    ...
    try:
        ...
        for dept in sub_departments:
            results, totals = process_lis_department_data(df, dept)
            all_results[dept] = {'data': results, 'totals': totals}
            # plots[dept] = generate_lis_visualizations(results)
...
def process_lis_department_data(df, sub_dept):
    base_locations = [
    ...
    for loc in base_locations:
        row_data = {'Sl.No': sl_no, 'Ordering Location': loc}
        total_count = 0
...
def process_ris_excel(file7_path):
    ...
    if 'Patient Company' not in df_category_ris or 'Visit Type' not in df_category_ris:
        return (
                "Required columns 'Patient Company' or 'Visit Type' not found in the file. "
                f"Found columns: {df_category_ris.columns.tolist()}",
                400,
                )
    
    df_filtered_category = df_category_ris[df_category_ris['Ordering Location'].isin(main_departments) & df_category_ris['Patient Company'].isin(patient_category)]
    print(df_filtered_category['Visit Type'].unique())
    op_category_counts = (
    ...
    print(df_filtered_category['Visit Type'])
    print("ip")
    print(ip_category_counts)
    summary_table_ris = pd.DataFrame({
                'Patient Company': patient_category
            })
